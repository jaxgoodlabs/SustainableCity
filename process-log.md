...
o	Goal: Make all addresses and property names match corresponding records in Cartegraph data set.
...
o	2017 PM data + Cartegraph
o	2018 PM data + Cartegraph
o	2017 PM data + 2018 PM data
...
o	Disambiguate true matches (external duplicates) from false matches (internal duplicates)
...
o	Be careful of road names that contain the word “road” and correct any inadvertent changes (e.g., Broadway, etc.)
...
o	Save “overview 2017” as “2017 City Building Energy Data.” Archive original file.
o	Join “2017 City Building Energy Data” to “”
	Only 5-10% match on Address field between data sets ☹ 
	
o	Can’t match on Address – only 10-15% match with Pittsburgh Address map layer
o	Parcels…?  Will likely be multiple buildings on a single parcel, have to disambiguate with Google Maps and assign unique ID to differentiate (BL_ID)
...
o	PBBID – City Buildings don’t have PBBID
...
o	Eventually you should do this with the CMU buildings also.
o	Use a one to one join and then manually eliminate records for smaller buildings
...
o	Spatial Join results in very few joins. Problem may be with type of spatial join (looking for complete overlap or enclosure?) or incompatible geographic coordinate settings.
...
o	2017CoveredBuildings_Parcels map layer include BlockLot ID which can be joined to Facilities map layer (no BlockLot ID in Geoffrey’s Cartegraph data; no PIN in Facilities map layer)
...
o	AddressCvdBldgs_Intersect (green circle, 6 pt) – Intersecting records between CoveredBuildingsList_Addresses and CoveredBuildings2017_SpatialJoin map layers. 
o	AddressPittBldgs_Intersect (orange circle, 6 pt) – Intersecting records between CoveredBuildingsList_Addresses and PittsburghBuildings map layers.
o	UnmatchedAddresses (red circle, 6 pt) – Non-intersecting records between CoveredBuildingsList_Addresses and PittsburghBuildings or CoveredBuildings2017_SpatialJoin map layers. 
...
o	Result: Action results in only 146 records. 9 records unmatched. 8 of these are partially unmatched while the remaining 1 record (Schenley Museum) is completely unmatched. 
...
o	Test 1b: Try inner join on address field.
...
o	Result: This results in 168 records returned. This likely owes to duplicate address names (without street number).
o	Test 1d: Rerun inner join on Property Name field.
...
o	Action results in 139 records (153-139 = 14 missing records).
...
o	Optional: Use definition query to only display 14 missing buildings listed above.
...
o	Building ID (all applicable)  Primary key
	Object ID (CMU)
	BL_ID (CMU)
	PBBID (Energy Star Portfolio Manager)
o	Property Name (Energy Star Portfolio Manager)
...
o	Parcel ID / PIN / Map Block Lot
o	Total CO2e emissions by year (metric tons CO2e)
	2017 (baseline)
	Etc.
o	Change in CO2e from 2017 baseline (metric tons CO2e)
o	Percent change in CO2e from 2017 baseline
o	Emissions intensity (kgCO2e/ft²)
	2017 (baseline)
	Etc.
o	Change in Emissions Intensity from 2017 baseline (kgCO2e/ft²)
o	Percent change in Emissions Intensity from 2017 baseline (kgCO2e/ft²)
o	Site EUI (unit)
	2017 (baseline)
	Etc.
o	Change in Site EUI from 2017 baseline (unit?)
o	Percent change in Site EUI from 2017 baseline
o	Source EUI (unit?)
	2017 (baseline)
	Etc.
o	Change in Source EUI from 2017 baseline (unit?)
o	Percent change in Source EUI from 2017 baseline
o	Compliance Status
o	Area (sq ft)
o	
...
o	PBBID  Energy Star Portfolio Manager (Flore created)
o	BL_ID  Carnegie Mellon University Facilities Management Services
o	OBJECTID  Allegheny County?
o	Campus ID  Energy Star Portfolio Manager (Flore created)?
o	ADDRESS_1  Allegheny County? 
o	ADDRESS_2  
o	PARCEL_ID / PIN  Allegheny County?
o	MAP_BLOCK_LOT  Allegheny County?
o	 Cartegraph
o	COMPLIANCE_STATUS  Covered Building List + Energy Star Portfolio Manager
...
o	Alternatively, run a join in ArcGIS and then export the resulting table as a excel worksheet. 
...
o	2017 Covered Building List must be:
...
o	Establish the primary key for the Covered building list. 
...
o	PERCENT_CHANGE_CO2e is not null; and
o	PERCENT_CHANGE_CO2e is less than 6428.72; and
o	PERCENT_CHANGE_CO2e is not equal to 1; and
o	PERCENT_CHANGE_CO2e is not equal to -1.
